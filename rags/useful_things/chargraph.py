...
class APIClient:
    def __init__(
        self, api_key: str, use_openrouter: bool = False, model: Optional[str] = None
    ):
...
5. Portrait Generation:
   - For each character, create a detailed prompt for AI image generation that captures:
    ...
    def get_schema(
        self, desc_sentences: Optional[int] = None, generate_portraits: bool = False
    ) -> Dict[str, Any]:
                            ...
                            "response_format": {
                                ...
                                "json_schema": {
                                    ...
                                    "schema": self.get_schema(
                                        desc_sentences, generate_portraits
                                    ),
                                },
                            },
                        },
...
class CharacterExtractor:
    ...
    def plot_network(self, G: nx.Graph, image_file: str) -> None:
        ...
        nx.draw_networkx_edges(
            G, pos, alpha=0.2, width=edge_widths, edge_color=edge_colors
        )
        ...
        plt.plot(
            [], [], "o", color="#4ECDC4", label="Supporting Characters", markersize=15
        )
    ...
    def process_text(
        ...
        input_file: str,
        output_file: str,
        previous_json_file: Optional[str] = None,
        iterations: int = 1,
        delay: int = 300,
        ...
        for i in range(iterations):
            ...
            for attempt in range(max_retries):
                try:
                    messages = self.api_client.create_messages(
                        text, previous_json, desc_sentences, generate_portraits, copies
                    )
                    result = self.api_client.make_request(
                        messages, desc_sentences, generate_portraits, temperature
                    )
                    ...
                    debug_filename = str(
                        Path(output_filename).with_suffix(".debug.txt")
                    )
                    ...
                    if not isinstance(content["characters"], list) or not isinstance(
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"Failed to process iteration {i + 1} after {max_retries} attempts"
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract characters and their relationships from text."
    )
    ...
    parser.add_argument(
        "-prev", "--previous", help="Previous JSON file to use as initial data"
    )
    parser.add_argument(
        "-plot",
        "--plot_graph",
